...
const DayWorkerValidation = (data) => {
  let errors = {};
  ...
  } else if (!moment(term_start_date, "YYYY-M-D", true).isValid()) {
    // ||
  ...
  if (checkEmpty(working_hours_from) || checkEmpty(working_hours_end)) {
    errors.require = true
  ...
  if (
    !checkEmpty(break_time_from, { ignore_whitespace: true }) ||
    !checkEmpty(break_time_end, { ignore_whitespace: true })
  ) {
...
export default DayWorkerValidation;
