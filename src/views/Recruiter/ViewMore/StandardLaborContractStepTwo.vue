          ...
          <div class="container">
            <div class="form-group mb-0">
              <div class="business-period-cont">
                ...
                <div class="period-row from-row mb-16">
                  <ul
                    :class="`date-info ${
                    ...
                    <li>
                      <span
                        ><input
                          ...
                          @change="(e) => (startDate.day = e.target.value)"
                        /> </span
                      >일
                    </li>
                  </ul>
      ...
      <div class="back-wh border-top">
        <div class="cm-top-space pb-32">
          <div class="container">
            ...
            >
            <!-- @blur="
                    salary = commonStore.methods.formatSalary(
                      salary !== '' ? salary.replaceAll(',', '') : salary
                    )
                  " -->
                  ...
                  <div class="tabs">
                    <div class="tabs-otr">
                      <ul class="tabs-header">
                        <li
                          :class="`${bonus.defaultVal == 0 ? 'active' : ''}`"
                          @click="setBonusState(0)"
                        >
                          <span>상여금 없음</span>
                        </li>
                        <li
                          :class="`${bonus.defaultVal == 1 ? 'active' : ''}`"
                          @click="setBonusState(1)"
                        >
                          <span>상여금 있음</span>
                        </li>
                      </ul>
                    </div>
                    <div class="tab-content" v-show="bonus.defaultVal == 1">
                      <div class="outerside">
                  ...
                  <div class="tabs">
                    <div class="tabs-otr">
                      <ul class="tabs-header">
                        <li
                          :class="`${
                            other_salary.defaultVal == 0 ? 'active' : ''
                          }`"
                          @click="getOtherSalaryStatus(0)"
                        >
                          <span>기타급여 없음</span>
                        </li>
                        <li
                          :class="`${
                            other_salary.defaultVal == 1 ? 'active' : ''
                          }`"
                          @click="getOtherSalaryStatus(1)"
                        >
                          <span>기타급여 있음</span>
                        </li>
                      </ul>
                    ...
                    <div
                      class="tab-content"
                      v-show="other_salary.defaultVal == 1"
                    >
                      <div class="outerside">
                        <div class="tab-inner-cont">
                          <div class="form-group mb-0">
        ...
        <div class="cm-top-space pb-32">
          <div class="container">
            <div class="form-group mb-0 wage-pay-wrap">
              ...
              <p v-if="errors && errors?.wage_payment_type" class="error-msg">
                {{ errors?.wage_payment_type }}
              </p>
...
import StandardContractValidation from "@/Validations/StandardContract.js";
...
export default {
  ...
  setup() {
    ...
    const formDetail = async () => {
      ...
      break_time.from =
        res.data.break_time_from == "null" || res.data.break_time_from == null
          ? ""
          : res.data.break_time_from;
      break_time.till =
        res.data.break_time_end == "null" || res.data.break_time_end == null
          ? ""
          : res.data.break_time_end;
      console.log(typeof res.data.break_time_from);
    ...
    watch(
      () => wage_payment_type.value,
      (val) => {
        if (val != "monthly") {
          wage_payment_date_monthly.value = "";
        }
      }
    );
    ...
    function acceptNumber() {
      var x = companyPhoneNo.value
        .replace(/\D/g, "")
        .match(/(\d{0,3})(\d{0,4})(\d{0,4})/);
      console.log(x);
      companyPhoneNo.value = !x[3]
        ? x[2]
        : !x[2]
        ? x[1]
        : x[1] + (x[2] ? "-" + x[2] : "") + (x[3] ? "-" + x[3] : "");
    ...
    const addOtherSalary = () => {
      if (other_salary.salaries.length < 4) {
    ...
    const setAmount = (event, id) => {
      ...
      other_salary.salaries.map((ele, index) => {
        if (index === id) {
          return (ele.amount = String(event.target.value));
        }
      });
    ...
    const checkError = (data) => {
      const { isInvalid, error } = StandardContractValidation(data);
      if (error.require) {
    ...
    const isSubmit = async () => {
      ...
      let inputData = {
        ...
        days_off: filtered_days == null ? "" : filtered_days,
        pay_type: wage.value,
        salary: String(salary.value),
        bonus: bonus.status,
        bonus_amount: bonus.amount == null ? "" : String(bonus.amount),
    ...
    //   document.querySelectorAll(".timeField").forEach(
    //     (e) =>
    //       (e.oninput = () => {
    //         // Always 2 digits
