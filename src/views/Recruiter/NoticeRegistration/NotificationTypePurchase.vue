<template>
  <BottomSheetCustom v-if="commonstore.state.isModalNotificationTypePurchase">
  <div :class="`back-wh common-sec ${commonstore.state.currentThemeStore}`">
    <div class="top-hdr top-hdr-new">
        <div class="container">
          <h4 class="heading">

            <!-- <a v-if="Object.keys($route.query).length === 0" @click="$emit('closeModal')" class="cross-arrow"
              ><img src="@/assets/icons/cross-icon2.svg" alt="" /></a> -->
            공고유형 구매
            <a
            href="javascript:void(0);"
            @click="commonstore.state.showPaidProductApplicationModal = true"
    ...
    <div class="container">
      <div class="pb-32">
        <div class="sub-otr no-border">
          ...
          <p>
            공고 등록 시 적용할 다양한 공고유형을 확인해 보세요! <br />
            공고유형을 적용하여 다양한 혜택을 경험해보세요! <br />
            보유중인 유료 상품은 오른쪽 상단의 유료상품 보관함 아이콘을 <br /> 눌러 사용하실 수 있습니다.
          </p>
        </div>
      </div>
    </div>
    ...
    <NotificationRegistrationComplete
      ...
      @confirm="$router.push({ name: 'JobListing' }), commonstore.state.isModalNotificationTypePurchase = false"
    />
    <PaidProductPayment
      v-if="commonstore.state.showPaidProductPaymentModal"
      :productDetails="addToCartData"
      :balance="balance"
      @confirm="confirmPayment"
    />
    <PaidProductApplication
      v-if="commonstore.state.showPaidProductApplicationModal"
    ...
    <div class="scrapped-icon scrapped-icon-transform act" v-if="popupMessageShow">
      ...
      <div class="scrapped-icon scrapped-icon-transform act" v-if="popupMessage">
        <p>
          <span><img src="@/assets/icons/scrapped-icon.svg" alt="" /></span>
          이미 추가된 상품입니다.
        </p>
...
export default {
  ...
  setup() {
    ...
    watch(() => commonstore.state.showPaidProductPaymentModal, async (first, second) => {
      if (commonstore.state.showPaidProductPaymentModal === true) {
    ...
    watch(() => commonstore.state.showPaidProductApplicationModal, async (first, second) => {
      if (commonstore.state.showPaidProductApplicationModal === true) {
          var body = document.querySelector("html");
          body.style.overflow = "hidden";
        } else {
          var body = document.querySelector("html");
    ...
    const clickToPurchase = () => {
      if (choosenType.value !== "free") {
        commonstore.state.showPaidProductPaymentModal = true;
      } else {
    ...
    const removeFromCart = (i) => {
      ...
      if(addToCartData.value.length === 1 && addToCartData.value[0].ad_type_id === 6) {
        addToCartData.value = []
      }
      totalAmount.value = addToCartData.value.reduce(
        (n, { discounted_price }) => n + discounted_price,
    ...
    const confirmPayment = async () => {
      try {
        ...
        commonstore.state.showPaidProductPaymentModal = false;
        let inputdata = commonstore.state.fourstepdata;
        inputdata.array_purchase_for_ad = array_purchase_for_ad;
        inputdata.array_storage_box_list = [];
        console.log(inputdata);
    ...
    const confirmApply = async (addToUseData) => {
      try {
        ...
        commonstore.state.showPaidProductApplicationModal = false;
        let inputdata = commonstore.state.fourstepdata;
