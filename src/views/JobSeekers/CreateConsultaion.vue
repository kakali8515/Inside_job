    ...
    <form>
      <div class="form-wrapper">
        <form>
            ...
            <div class="cm-top-space pb-32">
              <div class="container">
                ...
                <div class="form-group mb-0">
                  <input
                    type="text"
                    v-model.trim="consultationOfLaborData.title"
                    class="form-control"
                    placeholder="제목을 입력해주세요"
                  />
                  <small v-if="errors && errors.title" class="error-msgsign">{{
                    errors.title
                  }}</small>
                </div>
              </div>
            ...
            <div class="cm-top-space pb-32">
              <div class="container">
                ...
                <div class="form-group mb-0">
                  ...
                  <small
                    v-if="errors && errors.consultation"
                    class="error-msgsign"
                    >{{ errors.consultation }}</small
                  >
                </div>
              </div>
            </div>
          ...
          <div class="back-wh border-top">
            <div class="cm-top-space pb-32">
              <div class="container create-cons-area">
                <div class="sub-otr b-color">
                  <h3 class="sub-heading mb-0">비밀글 설정</h3>
                </div>
                <div class="check-list squre mb-24">
                  <label class="check-input">
                    해당 문의글을 <span>비밀글</span>로 설정하시겠습니까?
                    <input type="checkbox" v-model="locked" />
                    <span class="checkmark"></span>
                  </label>
                </div>
                <div class="form-group" v-if="locked == true">
                  <label class="labl-mb-4">비밀번호 입력</label>
                  <div class="password-input">
                    <input
                      :type="passwordFieldType"
                      v-model="consultationOfLaborData.password"
                      maxlength="16"
                      class="form-control"
                      placeholder="**********"
                    />
                    ...
                    /></span>
                    <span @click="switchVisibility" v-else class="eye"
                      ><img src="@/assets/icons/eye-open.svg" alt=""
                    /></span>
                    <small
                      v-if="passwordError !== '' && locked == true"
                      class="error-msgsign"
                      >{{ passwordError }}</small
                    >
                  </div>
                ...
                <div class="btnn-group">
                  ...
                  <div class="btn-group-innr">
                    ...
                    <!-- :disabled ="(locked == false && (consultationOfLaborData.title == '' || consultationOfLaborData.consultation == '')) || (locked == true && (consultationOfLaborData.password == '' || consultationOfLaborData.consultation == '' || consultationOfLaborData.title == ''))" -->
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  setup() {
    ...
    const setSelfIntro = (event) => {
      ...
      console.log(
        consultation.value.value.length,
        consultation.value.maxLength
      );
      ...
      if (event.key === "Enter") {
        ...
        selfIntro.value =
          selfIntro.value.substring(0, start) +
          "\n" +
          selfIntro.value.substring(end);
        // Set the new cursor position after the line break
    ...
    const updateConsultation = async (data) => {
      try {
        let res = await ConsultationOfLaborService.editConsultation(
          route.query.id,
          data
        );
    ...
    const saveConsultation = async (data) => {
      try {
        let res = await ConsultationOfLaborService.addConsultation(data);
    ...
    const switchVisibility = () => {
      passwordFieldType.value =
        passwordFieldType.value === "password" ? "test" : "password";
    };
